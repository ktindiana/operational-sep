...
#### all_program_info()
    ...
    FirstStart, LastEnd, ShortEvent, LateHundred, sep_year, sep_month,     sep_day = sep.run_all(start_date, end_date, experiment, flux_type,         model_name, user_file, showplot, saveplot, detect_prev_event,          two_peaks, umasep, threshold)
    ...
    Added functionality to output the >10 MeV and >100 MeV time series for the
    ...
    NOTE: Put your flux file into the "datapath" directory. Filenames will be relative to this path.
...
#### calculate_event_info(energy_thresholds, flux_thresholds, dates, integral_fluxes, detect_prev_event, two_peaks, is_diff_thresh)
...
#### calculate_onset_peak(experiment, energy_thresholds, dates, integral_fluxes, crossing_time, event_end_time, showplot)
...
#### calculate_umasep_info(energy_thresholds, flux_thresholds, dates, integral_fluxes, crossing_time)
    Uses the integral fluxes (either input or estimated from differential
    channels) and all the energy and flux thresholds set in the main program
...
#### check_bin_exists(threshold, energy_bins)
...
#### check_ephin_data(startdate, enddate, experiment, flux_type)
...
#### check_sepem_data(startdate, enddate, experiment, flux_type)
...
#### get_fluence_spectrum(experiment, flux_type, model_name, energy_threshold, flux_threshold, sep_dates, sep_fluxes, energy_bins, is_diff_thresh, save_file)
...
#### print_values_to_file(experiment, flux_type, model_name, energy_thresholds, flux_thresholds, crossing_time, onset_peak, onset_date, peak_flux, peak_time, rise_time, event_end_time, duration, integral_fluences, is_diff_thresh, umasep, umasep_times, umasep_fluxes)
...
#### read_in_ephin(experiment, flux_type, filenames1)
...
#### read_in_goes(experiment, flux_type, filenames1, filenames2, filenames_orien)
    Read in GOES data from your computer.

    
#### read_in_sepem(experiment, flux_type, filenames1)
...
#### report_threshold_fluences(experiment, flux_type, model_name, energy_thresholds, energy_bins, sep_dates, sep_fluxes)
...
#### run_all(str_startdate, str_enddate, experiment, flux_type, model_name, user_file, showplot, saveplot, detect_prev_event, two_peaks, umasep, str_thresh)
